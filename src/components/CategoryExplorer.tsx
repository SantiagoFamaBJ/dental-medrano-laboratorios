...
import type { Marca, Producto } from "@/lib/types";
...
export default function CategoryExplorer({ productos, marcas = [] }: CategoryExplorerProps) {
  ...
  const marcasPresentes = useMemo(() => {
    const nombres = Array.from(new Set(productos.map((p) => p.marca).filter(Boolean))) as string[];
  ...
  const tecnologias = useMemo(
    () => Array.from(new Set(productos.map((p) => p.tecnologia).filter(Boolean))) as string[],
    [productos]
  );
  ...
  }, [productos, subcategoria, tecnologia, query]);

  const activeCount = [subcategoria, tecnologia].filter(Boolean).length;
    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-[240px_1fr]">
        <aside className="hidden lg:block">
          <div className="sticky top-28 rounded-2xl border border-mist-200 bg-white p-6">{FilterContent}</div>
        </aside>
